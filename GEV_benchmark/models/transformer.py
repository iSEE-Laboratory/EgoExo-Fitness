...
class TransformerDecoder(Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, return_attn=False):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Optional[Tensor] = None,
        ...
        # lay_id = -1
        output = tgt
        # has_nan = torch.isnan(tgt)
        # if has_nan.any():
        #     print('layer:', lay_id)
        #     print('target', output)
        #     assert 1==2
        # has_nan = torch.isnan(memory)
        # if has_nan.any():
        #     print('layer:', lay_id)
        #     print('memory', output)
        #     assert 1==2
        attns = []
        for lay_id, mod in enumerate(self.layers):
            if self.return_attn:
...
class TransformerDecoderLayer(Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu",
                 layer_norm_eps=1e-5, batch_first=False, device=None, dtype=None, return_attn=False) -> None:
        factory_kwargs = {'device': device, 'dtype': dtype}
